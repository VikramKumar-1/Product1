...
import { useRegister } from "../hooks/partner/useRegister";
import { useKyc } from "../hooks/partner/usekyc";
...
const GLOBAL_STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    --blue: #2563eb; --blue-light: #eff6ff; --blue-border: #dbeafe;
    ...
    --red: #ef4444; --red-light: #fef2f2; --amber: #d97706; --amber-light: #fffbeb;
  ...
  .cm-input-shell:focus-within { border-color:var(--blue); background:var(--white); box-shadow:0 0 0 4px rgba(37,99,235,.08); }
  ...
  .cm-input-shell input, .cm-input-shell select, .cm-input-shell textarea {
    ...
    font-family:'Inter',sans-serif; min-width:0; width:100%; box-sizing:border-box;
  }
  .cm-input-shell select { appearance:none; cursor:pointer; }
  .cm-input-icon { position:absolute; left:14px; top:50%; transform:translateY(-50%); color:#9ca3af; pointer-events:none; display:flex; flex-shrink:0; }
  .cm-label { display:block; font-size:11px; font-weight:700; color:#6b7280; letter-spacing:.07em; text-transform:uppercase; font-family:'Inter',sans-serif; margin-bottom:7px; }
  .cm-error-msg { margin:5px 0 0 2px; font-size:12px; color:var(--red); display:flex; align-items:center; gap:4px; font-family:'Inter',sans-serif; }
  .cm-btn-primary {
    ...
    transition:all .25s; box-shadow:var(--shadow-blue); letter-spacing:.01em; box-sizing:border-box;
  }
  .cm-btn-primary:hover:not(:disabled) { background:#1d4ed8; transform:translateY(-2px); box-shadow:0 8px 28px rgba(37,99,235,.38); }
  ...
  .cm-step-dot { width:28px; height:28px; border-radius:50%; display:flex; align-items:center; justify-content:center; transition:all .35s cubic-bezier(.16,1,.3,1); font-size:11px; font-weight:800; }
  .cm-pay-opt { border:1.5px solid var(--border); border-radius:var(--radius-lg); padding:16px; cursor:pointer; transition:all .2s; display:flex; align-items:center; gap:12px; background:var(--surface); box-sizing:border-box; }
  ...
  .cm-right { margin-left:44%; width:56%; min-height:100vh; display:flex; flex-direction:column; align-items:center; background:var(--white); overflow-y:auto; }
  .cm-mobile-header { display:none; }
  @media(max-width:900px){
    #partner-root { position:static !important; height:auto !important; overflow-x:hidden !important; overflow-y:auto !important; inset:auto !important; }
    ...
    .cm-right { margin-left:0 !important; width:100% !important; max-width:100% !important; min-height:unset !important; height:auto !important; overflow-x:hidden !important; overflow-y:visible !important; align-items:stretch !important; }
    .cm-shell { width:100% !important; max-width:100% !important; padding:20px 16px 80px !important; box-sizing:border-box !important; overflow-x:hidden !important; }
    .cm-input-shell { width:100% !important; max-width:100% !important; box-sizing:border-box !important; }
    .cm-input-shell input, .cm-input-shell select, .cm-input-shell textarea { min-width:0 !important; max-width:100% !important; width:100% !important; box-sizing:border-box !important; }
    .cm-grid-2 { grid-template-columns:1fr !important; }
    .cm-pay-grid { grid-template-columns:1fr 1fr !important; }
    .cm-pay-opt { padding:12px 10px !important; gap:8px !important; }
    .cm-upload-row { flex-direction:column !important; }
    .cm-success-grid { grid-template-columns:1fr 1fr 1fr !important; }
    .cm-bank-grid { grid-template-columns:1fr !important; }
  }
...
function Spinner({ color = "#fff" }: { color?: string }) {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" style={{ animation: "cm-spin .75s linear infinite", flexShrink: 0 }}>
    ...
    <div style={{ width: "100%", boxSizing: "border-box" }}>
      ...
      <div className={`cm-input-shell${error ? " error" : ""}`}>
        ...
        <button type="button" onClick={() => setShow(s => !s)}
          style={{ position: "absolute", right: "12px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#9ca3af", display: "flex", padding: "4px", borderRadius: "6px", flexShrink: 0 }}
  ...
  return (
    <div style={{ width: "100%", boxSizing: "border-box" }}>
      ...
      <div className={`cm-input-shell${error ? " error" : ""}`}>
        ...
        <span style={{ position: "absolute", right: "12px", top: "50%", transform: "translateY(-50%)", pointerEvents: "none", color: "#9ca3af", flexShrink: 0 }}>
  ...
  return (
    <div style={{ width: "100%", boxSizing: "border-box" }}>
      ...
      <div className={`cm-input-shell${error ? " error" : ""}`}
        ...
        <span style={{ position: "absolute", left: "14px", top: "14px", color: "#9ca3af", flexShrink: 0 }}>
        ...
        <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={3}
          style={{ flex: 1, width: "100%", border: "none", background: "transparent", outline: "none", fontSize: "14px", color: "var(--text-primary)", padding: "14px 14px 14px 42px", resize: "none", height: "auto", fontFamily: "'Inter',sans-serif", boxSizing: "border-box" }} />
      </div>
      {error && <FieldError message={error} />}
    </div>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "12px" }}>
      <div style={{ position: "relative" }}>
        <div onClick={handleClick}
          ...
          style={{ width: "104px", height: "104px", borderRadius: "50%", cursor: locked ? "not-allowed" : "pointer", border: `2.5px ${preview ? "solid" : "dashed"} ${drag ? "var(--blue)" : preview ? "var(--blue)" : "#fca5a5"}`, background: drag ? "var(--blue-light)" : preview ? "var(--blue-light)" : "var(--red-light)", position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .22s", boxShadow: preview ? "0 0 0 4px rgba(37,99,235,.1)" : "0 0 0 3px rgba(239,68,68,.08)", flexShrink: 0 }}>
          {preview ? (
            <>
              <img src={preview} alt="Profile" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%" }} />
              <div style={{ position: "absolute", inset: 0, borderRadius: "50%", background: "rgba(0,0,0,.4)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: locked ? 1 : 0, transition: "opacity .2s" }}>
      ...
      <div style={{ textAlign: "center" }}>
        <p style={{ fontSize: "12px", fontWeight: 700, color: "var(--text-secondary)", margin: "0 0 2px 0" }}>Profile Photo <span style={{ color: "#ef4444" }}>*</span></p>
...
function UploadZone({ label, side, error, onChange }: { label: string; side: "front" | "back"; error?: string; onChange: (f: File) => void }) {
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, position: "relative", width: "100%", boxSizing: "border-box" }}>
      <FieldLabel text={label} />
      <div onClick={handleClick}
        onDragOver={e => { if (locked) return; e.preventDefault(); setDrag(true); }}
        onDragLeave={() => setDrag(false)}
        onDrop={e => { e.preventDefault(); setDrag(false); if (!locked) { const f = e.dataTransfer.files[0]; if (f) handleFile(f); } }}
        style={{ borderRadius: "var(--radius-lg)", border: `1.5px ${locked ? "solid" : "dashed"} ${error ? "#fca5a5" : drag ? "var(--blue)" : preview ? "#22c55e" : "var(--border)"}`, background: drag ? "var(--blue-light)" : preview ? "var(--green-light)" : "var(--surface)", minHeight: "148px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: locked ? "not-allowed" : "pointer", position: "relative", overflow: "hidden", transition: "all .2s", width: "100%", boxSizing: "border-box" }}>
        {preview ? (
          <>
        ...
        return (
          <div key={s.n} style={{ display: "flex", alignItems: "center", flex: i < steps.length - 1 ? 1 : "initial" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "4px" }}>
              <div className="cm-step-dot" style={{ background: done ? "#22c55e" : active ? "var(--blue)" : "var(--surface)", border: `2px solid ${done ? "#22c55e" : active ? "var(--blue)" : "var(--border)"}`, color: done || active ? "#fff" : "#9ca3af", boxShadow: active ? "0 0 0 3px rgba(37,99,235,.12)" : "none" }}>
                {done
    ...
    <div style={{ display: "flex", flexDirection: "column", width: "100%", boxSizing: "border-box" }}>
      <div>
        ...
        <div className={`cm-input-shell${error ? " error" : ""}`}>
          ...
          <span style={{ position: "absolute", right: "12px", top: "50%", transform: "translateY(-50%)", flexShrink: 0 }}>
            {fetching && <Spinner color="#2563eb" />}
            {!fetching && stateValue && <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>}
            {!fetching && failed && value.length === 6 && <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>}
          </span>
        </div>
        {error ? <FieldError message={error} /> : failed && value.length === 6 && <p style={{ margin: "5px 0 0 2px", fontSize: "11.5px", color: "#f59e0b", fontWeight: 600 }}>Pincode not found — enter state manually</p>}
      </div>
      {(stateValue || failed) && value.length === 6 && (
        <div className="cm-state-in" style={{ marginTop: "14px" }}>
          ...
          <div className="cm-input-shell" style={{ borderColor: stateValue ? "#22c55e" : "var(--border)", background: stateValue ? "var(--green-light)" : "var(--surface)" }}>
            ...
            {stateValue && !failed && (
              <span style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", gap: "3px", background: "#dcfce7", borderRadius: "6px", padding: "2px 8px", flexShrink: 0 }}>
...
function Banner({ icon, text, variant = "blue" }: { icon: React.ReactNode; text: React.ReactNode; variant?: "blue" | "green" | "amber" | "red" }) {
  ...
  return (
    <div style={{ background: c.bg, border: `1.5px solid ${c.border}`, borderRadius: "var(--radius-md)", padding: "12px 14px", display: "flex", alignItems: "flex-start", gap: "10px", width: "100%", boxSizing: "border-box" }}>
      <span style={{ color: c.color, flexShrink: 0, marginTop: "1px" }}>{icon}</span>
      <p style={{ fontSize: "12.5px", color: c.color, fontWeight: 500, lineHeight: 1.6, margin: 0, minWidth: 0 }}>{text}</p>
    </div>
  );
}
    ...
    <div className="cm-left">
      ...
      <div style={{ position: "relative", zIndex: 4 }}>
        <Link to="/" style={{ display: "inline-flex", alignItems: "center", gap: "10px", textDecoration: "none" }}>
          ...
          <span style={{ fontSize: "20px", fontWeight: 800, color: "#fff", letterSpacing: "-.5px", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>City<span style={{ color: d.accent }}>Mate</span></span>
...
function SectionCard({ title, onEdit, children }: { title: string; onEdit?: () => void; children: React.ReactNode }) {
  return (
    <div style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: "var(--radius-xl)", padding: "16px", width: "100%", boxSizing: "border-box" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
        ...
        {onEdit && (
          <button onClick={onEdit}
            style={{ background: "none", border: "1px solid var(--border)", cursor: "pointer", color: "var(--blue)", fontSize: "12px", fontWeight: 700, padding: "4px 12px", borderRadius: "var(--radius-sm)", transition: "all .15s", flexShrink: 0 }}
...
export default function Partner(): React.ReactElement {
  ...
  // OTP state — commented out, enable later:
  ...
  const { register, isPending: registerLoading } = useRegister({
    onSuccess: (data) => {
  ...
  const { kyc, isPending: kycLoading } = useKyc({
    onSuccess: () => { goStep(4); },
    onError: (error: unknown) => {
      const msg =
        (error as { response?: { data?: { message?: string } } })?.response?.data?.message ??
        "KYC verification failed. Please try again.";
      setErrors(e => ({ ...e, kycError: msg }));
    },
  });
  ...
  const handleNext = () => {
    if (step === 1 && v1()) register(form);
    else if (step === 2 && v2()) goStep(3);
    else if (step === 3 && v3()) kyc(form);
    else if (step === 4 && v4()) goStep(5);
    else if (step === 5) setSubmitted(true);
  };

  const handleReset = () => {
    setSubmitted(false); setStep(1); setAnimKey(0); setForm(INIT);
    setErrors({}); setProfilePreview(null);
  };
  ...
  if (submitted) {
    return (
      <div style={{ minHeight: "100vh", background: "var(--white)", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px", fontFamily: "'Inter',sans-serif" }}>
        ...
        <div style={{ textAlign: "center", maxWidth: "480px", width: "100%", animation: "cm-fadeUp .6s cubic-bezier(.16,1,.3,1) both", padding: "0 16px", boxSizing: "border-box" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: "20px" }}>
            <div style={{ position: "relative", animation: "cm-ringIn .55s .1s cubic-bezier(.16,1,.3,1) both" }}>
              <div style={{ width: "88px", height: "88px", borderRadius: "50%", overflow: "hidden", border: "3px solid var(--blue)", boxShadow: "0 0 0 5px rgba(37,99,235,.12), 0 8px 28px rgba(37,99,235,.2)", animation: "cm-glow 2.5s 1s ease-in-out infinite", flexShrink: 0 }}>
                {profilePreview
          ...
          <div className="cm-success-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px", marginBottom: "14px" }}>
            {[{ label: "Name", val: form.fullName, icon: "👤" }, { label: "Phone", val: `+91 ${form.phone}`, icon: "📱" }, { label: "City", val: form.city, icon: "📍" }].map(({ label, val, icon }) => (
              <div key={label} style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: "var(--radius-md)", padding: "12px 8px", boxSizing: "border-box" }}>
          ...
          <div style={{ display: "flex", gap: "10px", marginTop: "16px" }}>
            ...
            <button onClick={handleReset}
              style={{ flex: 1, height: "52px", borderRadius: "var(--radius-lg)", background: "var(--surface)", color: "var(--text-secondary)", border: "1.5px solid var(--border)", cursor: "pointer", fontSize: "14px", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", transition: "all .2s", boxSizing: "border-box" }}
  ...
  return (
    <div id="partner-root" style={{ fontFamily: "'Inter',sans-serif", display: "flex", height: "100vh", overflow: "hidden", position: "fixed", inset: 0 }}>
      <style>{GLOBAL_STYLES}</style>
      <LeftPanel step={step} />
      ...
      <div className="cm-right">
        ...
        {/* Form shell */}
        <div className="cm-shell" key={animKey} style={{ width: "100%", maxWidth: "440px", padding: "52px 0 80px", boxSizing: "border-box" }}>

          {/* ── Back button + Login link same row ── */}
          <div className="cm-f1" style={{ marginBottom: "24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            {step === 1
            ...
            {/* ← Login link — sirf step 1 pe, same row mein right side */}
            {step === 1 && (
              <Link to="/login"
                style={{ fontSize: "13px", fontWeight: 700, color: "var(--blue)", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: "5px", padding: "7px 14px", borderRadius: "var(--radius-sm)", background: "var(--blue-light)", border: "1px solid var(--blue-border)", transition: "all .2s" }}
                onMouseEnter={e => { e.currentTarget.style.background = "#dbeafe"; }}
                onMouseLeave={e => { e.currentTarget.style.background = "var(--blue-light)"; }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M15 3h4a2 2 0 012 2v14a2 2 0 01-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg>
                Login
              </Link>
            )}
          </div>
          ...
          <div className="cm-f3" style={{ marginBottom: "28px" }}>
            <div style={{ width: "48px", height: "48px", borderRadius: "var(--radius-md)", background: sm.bg, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "16px", color: sm.color, boxShadow: `0 4px 14px ${sm.color}22`, flexShrink: 0 }}>
              {sm.icon}
            </div>
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: "16px", width: "100%", boxSizing: "border-box" }}>
              <div className="cm-f4" style={{ background: "var(--surface)", border: `1.5px solid ${errors.profilePhoto ? "#fca5a5" : "var(--border)"}`, borderRadius: "var(--radius-xl)", padding: "20px 16px", boxSizing: "border-box" }}>
                ...
                {errors.profilePhoto && (
                  <p style={{ margin: "8px 0 0", textAlign: "center", fontSize: "12px", color: "var(--red)", display: "flex", alignItems: "center", justifyContent: "center", gap: "4px" }}>
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                    {errors.profilePhoto}
                  </p>
                )}
              </div>
              <div className="cm-f5" style={{ width: "100%" }}>
              ...
              <div className="cm-f5" style={{ width: "100%" }}>
                <Field label="Email Address" value={form.email} onChange={v => { setField("email", v); clearErr("email"); }} placeholder="e.g. rajesh@example.com" type="email" error={errors.email}
                  icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>} />
              </div>
              <div className="cm-f5" style={{ width: "100%" }}>
              ...
              <div className="cm-f6" style={{ width: "100%" }}>
                <PasswordField label="Password" value={form.password} onChange={v => { setField("password", v); clearErr("password"); }} placeholder="Create a password" error={errors.password} />
              </div>
              <div className="cm-f6" style={{ width: "100%" }}>
                <PasswordField label="Confirm Password" value={form.confirmPassword} onChange={v => { setField("confirmPassword", v); clearErr("confirmPassword"); }} placeholder="Repeat your password" error={errors.confirmPassword} />
              </div>
              {errors.submit && (
                <div className="cm-f6">
                  <Banner variant="red"
                    icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>}
                    text={errors.submit} />
                </div>
              )}
              <div className="cm-f7" style={{ width: "100%" }}>
                <button className="cm-btn-primary" onClick={handleNext} disabled={registerLoading}>
                  {registerLoading
          ...
          {step === 2 && (
            <div style={{ display: "flex", flexDirection: "column", gap: "16px", width: "100%", boxSizing: "border-box" }}>
              <div className="cm-f4" style={{ width: "100%" }}>
                <TextareaField label="Full Address" value={form.address} onChange={v => { setField("address", v); clearErr("address"); }} placeholder="House/Shop no., Street, Landmark…" error={errors.address} />
              </div>
              <div className="cm-f5 cm-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px", width: "100%", boxSizing: "border-box" }}>
          ...
          {step === 3 && (
            <div style={{ display: "flex", flexDirection: "column", gap: "16px", width: "100%", boxSizing: "border-box" }}>
              ...
              <div className="cm-f4" style={{ width: "100%" }}>
                ...
                <div className={`cm-input-shell${errors.aadhaarNumber ? " error" : ""}`}>
                  ...
                  <input type="text" inputMode="numeric"
                    ...
                    placeholder="XXXX XXXX XXXX" style={{ letterSpacing: ".12em", fontWeight: 600, paddingRight: "40px" }} />
              ...
              <div className="cm-f5 cm-upload-row" style={{ display: "flex", gap: "12px", width: "100%", boxSizing: "border-box" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <span style={{ fontSize: "10.5px", fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: ".07em", whiteSpace: "nowrap" }}>PAN Card (optional)</span>
                <div style={{ flex: 1, height: "1px", background: "var(--border)" }} />
              </div>
              <div className="cm-f5" style={{ width: "100%" }}>
                ...
                <div className="cm-input-shell" style={{ borderColor: form.panNumber && /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(form.panNumber.toUpperCase()) ? "#22c55e" : errors.panNumber ? "#fca5a5" : "var(--border)", background: form.panNumber && /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(form.panNumber.toUpperCase()) ? "var(--green-light)" : "var(--surface)" }}>
                  ...
                  <input type="text" value={form.panNumber} maxLength={10}
                    ...
                    placeholder="e.g. ABCDE1234F" style={{ letterSpacing: ".12em", fontWeight: 600, paddingRight: "80px" }} />
              ...
              <div className="cm-f6" style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: "var(--radius-md)", padding: "14px", boxSizing: "border-box" }}>
              ...
              {errors.kycError && (
                <div className="cm-f6">
                  <Banner variant="red"
                    icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>}
                    text={errors.kycError} />
                </div>
              )}
              <div className="cm-f7" style={{ width: "100%" }}>
                <button className="cm-btn-primary" onClick={handleNext} disabled={kycLoading}>
                  {kycLoading
                    ? <><Spinner />Verifying…</>
                    : <>Continue <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg></>}
                </button>
          ...
          {step === 4 && (
            <div style={{ display: "flex", flexDirection: "column", gap: "16px", width: "100%", boxSizing: "border-box" }}>
              <div className="cm-f4">
                ...
                <div className="cm-pay-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", width: "100%", boxSizing: "border-box" }}>
                  {[
              ...
              {form.paymentMethod === "upi" && (
                <div className="cm-f5" style={{ display: "flex", flexDirection: "column", gap: "12px", width: "100%" }}>
                  <Banner variant="blue"
              ...
              {form.paymentMethod === "bank" && (
                <div className="cm-f5" style={{ display: "flex", flexDirection: "column", gap: "12px", width: "100%" }}>
                  ...
                  <div className="cm-bank-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", width: "100%", boxSizing: "border-box" }}>
                ...
                <SectionCard title="Aadhaar KYC" onEdit={() => goStep(3)}>
                  ...
                  <div style={{ display: "flex", gap: "8px", marginTop: "4px" }}>
                    {[{ label: "Front Side", file: form.aadhaarFront }, { label: "Back Side", file: form.aadhaarBack }].map(({ label, file }) => (
                      <div key={label} style={{ flex: 1, background: file ? "var(--green-light)" : "var(--red-light)", border: `1px solid ${file ? "var(--green-border)" : "#fca5a5"}`, borderRadius: "var(--radius-sm)", padding: "8px", textAlign: "center", boxSizing: "border-box" }}>
                        <p style={{ margin: 0, fontSize: "9.5px", fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: ".06em" }}>{label}</p>
